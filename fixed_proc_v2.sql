...
CREATE OR REPLACE PROCEDURE DATAIKU_ELT_DEV.WORK_SPACE.GET_OBJECT_DDL_JS("P_CATALOG" VARCHAR, "P_SCHEMA" VARCHAR)
RETURNS VARIANT
LANGUAGE JAVASCRIPT
EXECUTE AS CALLER
AS $$
var errors = [];
var success_count = 0;
var tables_query = "SELECT DISTINCT TABLE_CATALOG, TABLE_SCHEMA, TABLE_NAME FROM \"" + P_CATALOG + "\".\"INFORMATION_SCHEMA\".\"COLUMNS\" WHERE TABLE_SCHEMA = ? AND COMMENT IS NOT NULL AND TABLE_SCHEMA != 'INFORMATION_SCHEMA' OR TABLE_NAME IN (SELECT TABLE_NAME FROM \"" + P_CATALOG + "\".\"INFORMATION_SCHEMA\".\"TABLES\" where comment is not null and comment <> '' and table_schema <> 'INFORMATION_SCHEMA')";
